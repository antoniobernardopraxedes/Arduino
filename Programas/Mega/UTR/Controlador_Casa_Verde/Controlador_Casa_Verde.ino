...
// energia da rede, o sistema entra em modo off grid, alimentando as cargas 1 e 2 e mudando a conexão de dois *
// paineis do inversor on grid para um controlador de carga. Se a carga 3 estiver habilitada e as condições   *
// satisfeitas, Ligar também a carga 3.                                                                       *
//                                                                                                            *
//*************************************************************************************************************
...
#define SD16            47  // O SD16 - 0 => Rede Normal / 1 => Falta CA Rede
...
void setup() {
  ...
  ModoOperacao = 0;        // Inicia o Modo de Operacao Economia
...
void VerificaTensaoRede() {
  ...
  if (VRede < VTRNFCA) {            // Se a tensao da rede cair abaixo do limite inferior,
    if (CTFCA > 0) {                // e se esta temporizando para confirmar Falta de CA,
      ...
      if ((Hr > 5) && (Hr < 18)) {
        digitalWrite(SD16,1);       // aciona a saida digital de falta CA para conectar os paineis fotovoltaicos
      }
    ...
    if (CTEA == 0) {               // Se terminou um ciclo de media de entradas analogicas,
      if (VRede > VTRFNCA) {       // Se a tensao da rede esta maior que o limite superior (voltou ao normal),
        ...
        if (CTRCA >= TRRede) {     // Se a tensao da rede se manteve acima do limite pelo tempo especificado,
          ...
          digitalWrite(SD16,0);    // e desativa a saida digital de falta CA,
  ...
  if (ModoComando == 0) {             // Se o Modo de Comando = Local ou vem do reset,
    ...
    if (ModoOperacao == 1) {          // Se o Modo Normal esta selecionado,
      ...
      if ((digitalRead(ChPrB) == 0) && (digitalRead(ChPrC) == 0)) {
        if (OpOnGrid) {                // Se é operação On Grid
          ModoControle1 = 1;
          ModoControle = 1;
...
void VerificaModoOp() {
  ...
  if (EAME0 >= VMinBat) {                 // Se a tensao 24Vcc esta OK, verifica todas as outras condicoes
    ...
    if (ModoOperacao == 1) {                // Se o Modo de Operacao = Normal,
      if (EstadoRede == 1) {                // e se a Tensao da Rede esta OK,
        ...
        if (OpOnGrid) {                     // Se é o modo de operação On Grid,
          if (Carga1 == 1) {                // Se a Carga1 esta Habilitada,
            if (ModoControle1 == 0) {       // e se o Modo de Controle da Carga 1 = Manual, 
                if (FalhaInversor2 == 0) {  // e se nao ha falha no Inversor 2,
                  ...
                  CT2_Inversor();           // e passa CT2 para o Inversor
                }
              }
            ...
            if (Carga2 == 1) {              // Se a Carga2 está Habilitada,
              if (ModoControle == 0) {      // e se o Modo de Controle = Manual para a Carga 2,
                if (FalhaInversor2 == 0) {  // e se nao ha falha no Inversor 2,
                  LigaInversor2(1,1,1,0);   // Liga o Inversor 2
                  CT1_Inversor();           // passa a CT1 para o Inversor.
                }
              }
              else {                          // Se o Modo de Controle = Automatico para a Carga 2,
            ...
            else {                          // Se a Carga2 está desabilitada,
              CT1_Rede();                   // passa a Carga2 para a Rede.
              if (Carga1 == 0) {            // Se a Carga1 está desabilitada,
                DesligaInversor2(1,1,1,0);  // Desliga o Inversor 2,
              }
            }
        ...
        // *** Se é o modo de operação Off Grid ***
        if (not OpOnGrid) {
      ...
      if (EstadoRede == 0) {                // Se ha falta de tensao da rede,
        if (FalhaInversor2 == 0) {          // e se nao ha falha no Inversor 2,
          LigaInversor2(1,1,1,0);           // Liga o Inversor 2
  ...
  // *** Se a tensão 24Vcc está baixa ***
  if (EAME0 < VMinBat) {               // Se a tensao 24Vcc esta baixa,
    CT2_Rede();                        // passa CT2 para a Rede
...
void CT2_Inversor() {
  ...
  if (digitalRead(SD17) == LOW) {  // Se a CT2 esta para a rede,
    if (FlgTmpIv2 == 0) {          // Se nao esta temporizando o inversor
      if (FlgTmpCT2 == 0) {          // e se a flag nao indica temporizacao,
        CTCT2 = NCTRCT2;             // carrega o contador de temporizacao,
        FlgTmpCT2 = 1;               // seta a flag indicando temporizacao em curso,
        digitalWrite(SD03, HIGH);    // desconecta a saida do Inversor da CT2.
